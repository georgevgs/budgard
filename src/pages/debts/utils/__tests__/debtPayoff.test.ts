...
describe('simulatePayoff', () => {
  it('returns empty result when no active debts', () => {
    ...
    expect(r.isUnpayable).toBe(false);
  });
  ...
  it('flags isUnpayable when minimum < monthly interest and no extra', () => {
    const r = simulatePayoff({
      debts: [
    ...
    expect(r.isUnpayable).toBe(true);
  });
...
describe('schedule reconciliation', () => {
  ...
  it('reconciles even when the minimum does not cover the interest', () => {
    ...
    expect(result.isUnpayable).toBe(true);
