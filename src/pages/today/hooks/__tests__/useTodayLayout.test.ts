...
beforeEach(() => {
  localStorage.clear();
  vi.mocked(todayApi.getLayout).mockResolvedValue(null);
  vi.mocked(todayApi.saveLayout).mockResolvedValue();
});
...
describe('useTodayLayout account sync', () => {
  it('hydrates the owner layout from the server', async () => {
    ...
    vi.mocked(todayApi.getLayout).mockResolvedValue(remote);
  ...
  it('seeds a missing server row from the existing device layout', async () => {
    ...
    await waitFor(() => {
      expect(todayApi.saveLayout).toHaveBeenCalledWith(local);
    });
  });
  ...
  it('keeps a local edit made while the initial fetch is in flight', async () => {
    ...
    expect(todayApi.saveLayout).toHaveBeenCalledWith(
  ...
  it('keeps the retry marker until the latest save succeeds', async () => {
    ...
    vi.mocked(todayApi.saveLayout).mockImplementation(
      () =>
