...
import { roundMoney, sumAmounts, toMinorUnits } from '@/constants/money';
...
export const useExpenseSplit = (
  ...
  const arePartsValid = parts.every(
    (part) => parseCurrencyInput(part.amount) > 0,
  );
  ...
  return {
    parts,
    remaining,
    canConfirm: arePartsValid && isSettled(remaining) && !isSaving,
    isSaving,
    updatePart,
    addPart,
    removePart,
