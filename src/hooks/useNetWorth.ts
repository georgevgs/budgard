...
export const useNetWorth = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const entries = await Promise.all(
        Array.from(required).map(async (key): Promise<[string, number]> => {
    ...
    accounts.forEach((a) => {
      const rate =
        a.default_currency === defaultCurrency
          ? 1
          : rates.get(RATE_KEY(a.default_currency, today)) ?? 1;
  ...
  }, [accounts, rates, defaultCurrency]);
