...
Status: released and complete. Netlify deploys the frontend from `main`; the
six migrations and four Edge Functions are on the linked project, the Auth
settings are applied and the push cron secret is rotated.
...
| F7 | `CRON_SECRET` lived in plain text in `cron.job.command`, and the job existed only in the dashboard. | The job reads the secret from Vault and is defined in a migration. `send-push-notifications` checks the token against Vault through `push_cron_secret_matches` (service role only, a boolean, digest comparison), so the secret has one copy and rotating it is a single `vault.update_secret()`. It was rotated. | 5, 6 |
| F8 | Hardening. | CSP drops the unused font CDN and adds `object-src 'none'` and `frame-ancestors 'none'`; `Cross-Origin-Opener-Policy: same-origin`; the service worker only opens same-origin paths from a notification; `supabase/.temp/` is untracked; every GitHub Action is pinned to the commit its major tag named, with the release in a comment, and `.github/dependabot.yml` proposes pin updates weekly in one grouped pull request, only for releases at least seven days old. | 5, 6 |
...
| ----- | ------ |
| Lint, `tsc -b` | Pass |
| Unit and invariant tests | Pass: 209 files, 1,814 tests |
...
| SQL boundary suite, linked project | Pass after `20260925150000`. Before `20260925140000`, failed at "Password session read receipts" (F9) |
| Push worker, linked project | A malformed token, a well-formed wrong one and the pre-rotation secret each get 401 from v21 |
...
bypass policy, which it refused by name. The push secret check was broken three
ways against the suite (granted to `authenticated`, accepting anything,
refusing everything) and each failed its own assertion.
